...
# the piece being fixated on, the chess board the fixation is being generated 
# in context of and whether or not the CHREST model constructing the Fixation 
# is experienced or not.
# 
# This test will therefore run 31 scenarios that are run with both an
# experienced and inexperienced CHREST model:
# 
# Scenario 1 
...
unit_test "constructor" do
  ...
  for scenario in 1..33
    for repeat in 1..2
      ...
      # when the time that the AttackDefenseFixation is decided upon is 
      # calculated.  Thus, if n is changed this test will break and performing 
      # this learning in a test causes unnecessary issues to deal with.
      #
      ...
      }.new(model_creation_time, false)

      # Set whether the CHREST model is experienced or not (not by default).
      ...
      time += 100
      if scenario != 3
        first_fixation = CentralFixation.new(time)
      ...
      # will always be the time the constructor was invoked plus 100 (the base 
      # time taken to generate a move).
      expected_time_decided_upon = time + 100

      # If the current scenario is >= 10 then piece moves will be attempted. The
      # time taken to move a piece across a Square on the chess board (mentally)
      # depends upon whether or not the CHREST model constructing the 
      # AttackDefenseFixation is experienced or not.  To keep code DRY, set the
      # time taken to move a piece across a Square on the chess board, even if
      # it won't be used.
      time_to_traverse_a_square = (model.isExperienced(time) ? 50 : 100)
      squares_traversed = 0
